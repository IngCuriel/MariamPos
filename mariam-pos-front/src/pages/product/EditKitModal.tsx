...
const EditKitModal: React.FC<EditKitModalProps> = ({
  isOpen,
  onClose,
  onSave,
  kit,
    ...
    if (isOpen) {
      ...
      if (kit) {
        ...
        setFormData({
          ...
          price: kit.price,
          cost: kit.cost || 0,
          category: kit.categoryId,
        });
        setStatus(kit.status);
        
        // Cargar items del kit
        if (kit.kitItems && kit.kitItems.length > 0) {
          setKitItems(kit.kitItems as KitItem[]);
        }
      }
    }
  }, [isOpen, kit]);
  ...
  const handleAddProductToKit = () => {
    ...
    const updated = [...kitItems, newItem];
    setKitItems(updated);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error?.response?.data?.error || "Error al actualizar el kit",
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container kit-modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          <h2>📦 Editar Kit/Combo</h2>
